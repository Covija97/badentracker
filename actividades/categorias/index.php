...
<main>
    ...
    <input type="text" id="searchInput" placeholder="Buscar categoría..." class="search-input" style="margin-left:1rem;">
    <br><br>
    <table class="table-main" id="catTable">
        <tr>
            <th>Nombre</th>
            <th>Descripción</th>
            <th>Acciones</th>
        ...
        <?php if ($query->num_rows > 0): ?>
            <?php while ($row = $query->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <a class="but align-left" href="categoria/delete.php?id=<?= htmlspecialchars($row['cat_id']) ?>" title="Borrar categoría" onclick="return confirm('¿Estás seguro de que deseas borrar esta categoría?');">
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <tr><td colspan="7">No hay categorias disponibles.</td></tr>
        <?php endif; ?>
    </table>
    <script>
    // Filtro de búsqueda en tabla de categorías
    document.getElementById('searchInput').addEventListener('input', function () {
        const filtro = this.value.toLowerCase();
        const filas = document.querySelectorAll('#catTable tr');
